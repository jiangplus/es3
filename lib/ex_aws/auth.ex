defmodule ExAws.Auth do
  ...
  def headers(http_method, url, service, config, headers, body, datetime \\ nil) do
    with {:ok, config} <- validate_config(config) do
      datetime = datetime || :calendar.universal_time()
  ...
  defp auth_header(http_method, url, headers, body, service, datetime, config) do
    ...
    # IO.inspect({http_method, url, query, headers, body, service, datetime, config})
  ...
  def signature(http_method, url, query, headers, body, service, datetime, config) do
    ...
    if config[:signer_url] && config[:signer_key] && config[:signer_url] != "" do
      sign_with_signer(http_method, path, query, headers, payload, service, datetime, config)
    else
      sign_with_keys(http_method, path, query, headers, payload, service, datetime, config)
    end
  end

  def sign_with_keys(http_method, path, query, headers, payload, service, datetime, config) do
    request = build_canonical_request(http_method, path, query, headers, payload)
